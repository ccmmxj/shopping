...
public final class TableDataFactory {
    ...
    public static <T> TableData<T> newInstaceSuccessResult(TableData<T> tableData,PageInfo<?> page,List<T> rows){
        return newInstaceResult(tableData,page,rows,200L,"获取成功");
    }
    public static <T> TableData<T> newInstaceFailResult(TableData<T> tableData,PageInfo<?> page,List<T> rows){
        return newInstaceResult(tableData,page,rows,500L,"获取失败");
    }

    public static <T> TableData<T> newInstaceResult(TableData<T> tableData,PageInfo<?> page,List<T> rows,Long code,String msg){
